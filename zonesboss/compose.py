...
def calque_props(zone_dir, seed):
    ...
    boites = []
    essais = 0
    while len(poses) < 9 and essais < 300:
        essais += 1
        o = objets[rng.integers(0, len(objets))]
        y = int(HORIZON + 12 + rng.random() ** 1.3 * (SCENE_H - HORIZON - 44))

        # Plus l'objet est bas, plus il est proche : il grandit. La taille
        # est bornée sur la PLUS GRANDE dimension — borner la seule hauteur
        # laissait les props larges (fûts couchés, arches) manger la moitié
        # de l'écran.
        prof = (y - HORIZON) / max(1, SCENE_H - HORIZON)
        cible = TILE * (0.85 + 0.75 * prof)
        ech = cible / max(o.shape[0], o.shape[1])
        wd = max(6, int(round(o.shape[1] * ech)))
        ht = max(6, int(round(o.shape[0] * ech)))
        x = int(rng.integers(2, max(3, SCENE_W - wd - 2)))

        # Deux props qui se chevauchent lisent comme un seul objet bancal :
        # on écarte la pose si elle mord une boîte déjà placée.
        b = (x, y - ht, x + wd, y)
        if any(not (b[2] <= c[0] or b[0] >= c[2] or
                    b[3] <= c[1] or b[1] >= c[3]) for c in boites):
            continue
        boites.append(b)
        poses.append((y, x, _resize(o, wd, ht)))

    # tri par profondeur : un prop plus bas passe devant
